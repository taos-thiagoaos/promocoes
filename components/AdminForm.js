...
export default function AdminForm({ scrapedData, isLoading, setIsLoading, initialData }) {
  ...
  const handlePreview = (e) => {
    ...
    if (!formData.title || !formData.startDate) {
      setStatus({
        message: 'Título, Data de Início e Imagem são obrigatórios para a pré-visualização.',
        error: true,
      });
      return;
    }
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      setStatus({
        message: 'Anúncio adicionado com sucesso! O site será reconstruído em breve.',
        error: false,
      });
