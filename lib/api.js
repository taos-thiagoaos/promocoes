...
function readJsonFile(fileName) {
  ...
  try {
    ...
    // Silencia o erro se o arquivo não existir, pois é esperado em alguns casos.
...
export function getAllPromos() {
  ...
  for (const fileName of promoFiles) {
    ...
    if (Array.isArray(promosFromFile)) {
      // Adiciona o slug a cada promoção no momento da leitura
      const promosWithSlugs = promosFromFile.map(promo => ({
        ...promo,
        slug: slugify(promo.title)
      }));
      allPromos = [...allPromos, ...promosWithSlugs];
    }
  }
...
export function getPromoBySlug(date, slug) {
  ...
  const promo = promos.find(p => slugify(p.title) === slug);
  return promo ? { ...promo, slug } : null;
}
