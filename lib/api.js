...
export function getAllPromos() {
  ...
  const promoFiles = fileNames.filter(
    (name) => name.startsWith('anuncios-') && name !== 'anuncios-fixos.json'
  );
...
export function getSuggestedPromos(currentPromoId) {
  ...
  const recentPromos = allPromos.filter(
    (promo) => new Date(promo.date) >= thirtyDaysAgo && promo.id !== currentPromoId
  );
