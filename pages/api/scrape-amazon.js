...
async function handler(req, res) {
  ...
  try {
    ...
    const isVerificationPage =
      htmlAmazonResponseText.includes('Clique no botão abaixo para continuar comprando') ||
    ...
    if (stillBlocked) {
      return res.status(403).json({
        error:
          'Amazon está bloqueando o acesso automatizado. Tente novamente mais tarde ou use uma VPN.',
      });
    }
    ...
    if (!title || !imageUrl) {
      return res.status(404).json({
        error:
          'Não foi possível extrair os dados. A estrutura da página da Amazon pode ter mudado.',
      });
    }
